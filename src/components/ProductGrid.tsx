    ...
    <section className="py-16 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mt-12 md:hidden">
          <Link 
            to="/products"
            className="inline-flex bg-green-500 text-white px-8 py-3 rounded-lg hover:bg-green-600 transition-colors font-semibold items-center gap-2 mx-auto"
          >
            عرض جميع المنتجات
            <ArrowLeft className="w-5 h-5" />
          </Link>
        ...
        <div className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { name: 'الأدوية', nameEn: 'Medicines', icon: '💊', count: '1,250+' },
            { name: 'الفيتامينات', nameEn: 'Vitamins & Supplements', icon: '🌟', count: '650+' },
            { name: 'العناية بالبشرة', nameEn: 'Skincare', icon: '✨', count: '420+' },
            { name: 'منتجات الأطفال', nameEn: 'Baby Products', icon: '👶', count: '380+' }
          ].map((category, index) => (
            <Link
              key={index}
